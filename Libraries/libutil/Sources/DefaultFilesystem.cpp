...
readSymbolicLink(std::string const &path, bool *directory) const
{
    ...
    /* Copy out of the packed reparse buffer to avoid unaligned access. */
    const BYTE *base = reinterpret_cast<const BYTE *>(data);
    const size_t path_offset_bytes =
        offsetof(REPARSE_DATA_BUFFER, SymbolicLinkReparseBuffer.PathBuffer) + offset * sizeof(WCHAR);
    std::vector<WCHAR> path_buffer;
    path_buffer.resize(length);
    std::memcpy(path_buffer.data(), base + path_offset_bytes, length * sizeof(WCHAR));

    WideString target = WideString(path_buffer.data(), path_buffer.data() + length);
